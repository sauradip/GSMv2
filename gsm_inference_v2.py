...
if __name__ == '__main__':
    ...
    temp_scale = config['training']['scale']
    ...
    ### Load Checkpoint ###
    checkpoint = torch.load(output_path + "/GSM_best_fuse.pth.tar")
    model.load_state_dict(checkpoint['state_dict'])
    model.eval()
    ### Load Dataloader ###
    test_loader = torch.utils.data.DataLoader(gsm_dataset.GSMDataset(subset="validation", mode='inference'),
    ...
    def post_process_multi(detection_thread,get_infer_dict):
        ...
        pred_data = pd.read_csv("gsm_output.csv")
        ...
        output_dict = {"version": "ANET v1.3, GSM", "results": detection_dict, "external_data": {}}

        with open(output_path + '/detection_result_nms{}.json'.format(nms_thres), "w") as out:
            json.dump(output_dict, out)

    def inferTAL(top_br_pred, bottom_br_pred, tscale):
    ...
    with torch.no_grad():
        ...
        for idx, input_data, input_data_2, input_data_4 in test_loader:
            video_name = test_loader.dataset.subset_mask_list[idx[0]]
            vid_count+=1
            input_data = input_data.cuda()
            input_data_2 = input_data_2.cuda()
            input_data_4 = input_data_4.cuda()
            if not os.path.exists(output_path + "/fig/"+video_name):
                os.makedirs(output_path + "/fig/"+video_name)

            # forward pass
            top_br_pred, bottom_br_pred, top_br_pred_2, bottom_br_pred_2, top_br_pred_4, bottom_br_pred_4 = model(input_data, input_data_2, input_data_4)

            prop_lst = inferTAL(top_br_pred, bottom_br_pred,temp_scale[0])
            prop_lst1 = inferTAL(top_br_pred_2, bottom_br_pred_2,temp_scale[1])
            prop_lst2 = inferTAL(top_br_pred_4, bottom_br_pred_4,temp_scale[2])

            # new_props.append(prop_lst)
            # new_props.append(prop_lst1)
            # new_props.append(prop_lst2)
            # new_props.append(prop_lst3)

            
        new_props = prop_lst2 + prop_lst + prop_lst1                     
        ...
        new_df.to_csv("gsm_output.csv", index=False)   
